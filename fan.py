...
def set_speed(speed=None):
    ...
    """ 
    return subprocess.check_output('echo level {0} | sudo tee "/proc/acpi/ibm/fan"'.format(speed), shell=True).decode('utf-8')
...
class MainApplication(tk.Frame):
    def __init__(self, parent, *args, **kwargs):
        ...
        row1.grid()
        for i in range(8):
            tk.Button(row1, text=str(i), command= lambda x=i:set_speed(x)).grid(row=0, column=i+1)
        
        row2 = tk.Frame()
        row2.grid()
        
        tk.Button(row2, text='Auto', command= lambda :set_speed('auto')).grid(row=0, column=0)
        tk.Button(row2, text='Full', command= lambda :set_speed('full-speed')).grid(row=0, column=1)
